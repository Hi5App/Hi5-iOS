...
//        print("its 3d size is \(size[0])*\(size[1])*\(size[2]) with \(size[3]) color channel(s)")
        ...
        let image4DSimple = image4DSimple(name: PBDimageURL.lastPathComponent, endiannessType: endianness, dataType: datatype, Int(size[0]), Int(size[1]), Int(size[2]), Int(size[3]), array128x128x128 ,array: decompressionByteBuffer,maxIntensity: 255,minIntensity: 0,avgIntensity: UInt8(0))
